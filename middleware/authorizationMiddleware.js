...
const authenticateJWT = (req, res, next) => {
    ...
    if (authHeader) {
        ...
        jwt.verify(token, process.env.JWT_SECRET_KEY, (err, data) => {
            ...
            const allowedFunctions = authorizationRules[data.role];
            if(allowedFunctions[functionName]!=undefined && allowedFunctions[functionName](req,res,data.email))
                next();
            else      
                return res.sendStatus(STATUS_CODES.FORBIDDEN)
            //if ima selektivnu propustljivost
                //next()
            //else
                //403
